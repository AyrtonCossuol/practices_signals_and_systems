...
Original file is located at
    https://colab.research.google.com/drive/1FXFUzcHda0Sz6XUNjav7tjc6SJ-cLmIv
"""
...
def main():
  ...
  w0 = 2 * np.pi * f     # frequencia angular
  ...
  Modx, wd = AnalisadorEspectro(x, t, 2**23, 1000, 'x(t)','|X(jw)|')

######################################################################################################
  # Fazendo a amostragem para Ts = 0.001
  Ts = T/10       # periodo do trem de impulso, periodo da amostragem
  t_primeira_ordem = np.arange(0, 6*T+1 , Ts)
  x_primeiroa_ordem = 100 * np.cos(w0 * t_primeira_ordem)

  # fazendo a reconstrução
  # com retentor de ordem um
  xr = np.interp(t, t_primeira_ordem, x_primeiroa_ordem)
...
######################################################################################################
  # Fazendo a amostragem para Ts = 0.0025
